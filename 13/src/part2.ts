...
try {
  ...
  packetPairsStr.push("[[2]]");
  packetPairsStr.push("[[6]]");

  packetPairsStr.sort((a, b) => {
    return -compareASTs(parsePacket(a).root, parsePacket(b).root);
  });

  const d2 = packetPairsStr.findIndex((pps) => pps === "[[2]]") + 1;
  const d6 = packetPairsStr.findIndex((pps) => pps === "[[6]]") + 1;

  console.log(d2, d6, d2 * d6);
